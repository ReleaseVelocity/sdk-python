...
async def test_graph_parallel_execution(mock_strands_tracer, mock_use_span):
    ...
    # Windows asyncio/event-loop overhead inflates short-sleep tests, so use a
    # more generous budget there. The non-Windows budget still proves parallelism
    # (sequential would be ~0.35s).
    parallel_budget = 1.0 if sys.platform == "win32" else 0.4
    assert total_time < parallel_budget, f"Expected parallel execution to be faster, took {total_time}s"
